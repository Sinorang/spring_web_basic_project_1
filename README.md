...
### 12. 사용자 탈퇴 후 데이터 완전 삭제 배치
- 탈퇴 처리된 사용자의 데이터 보존 정책 강화
- 탈퇴 처리(상태 변경) 후 90일이 경과한 사용자에 대해 아래 데이터 일괄 삭제
  - 해당 사용자가 작성한 모든 게시판(Board), 게시글(Post), 댓글(Comment), 플레이리스트(Playlist) 삭제
  - 모든 데이터 삭제 후 해당 사용자(User) 엔티티 삭제
- Spring Batch 등 스케줄러 기반 자동화 구현 예정

### 13. 정지/탈퇴 사용자 컨텐츠 노출 비활성화
- 정지 또는 탈퇴된 사용자가 작성한 게시판(Board), 게시글(Post), 댓글(Comment)은 서비스 내에서 노출되지 않도록 비활성화 처리
- 관리자 페이지 등에서만 별도 조회 가능하도록 구현
- 데이터는 삭제하지 않고, 상태 기반으로 노출 제어
